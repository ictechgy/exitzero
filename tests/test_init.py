"""Focused contracts for policy generation through ``aidd-gate init``."""
import json
...
class InitProfileTests(unittest.TestCase):
    ...
    def test_python_profile_roundtrips_paths_modules_and_commands(self):
        ...
        policy_path = self.root / "aidd-gate.toml"
        self.assertTrue(policy_path.is_file())
        from_aidd_gate = None
        sys.path.insert(0, str(ROOT / "packages" / "core" / "src"))
        try:
            from aidd_gate.policy import load_policy

            from_aidd_gate = load_policy(policy_path)
        finally:
            sys.path.pop(0)
        self.assertEqual(from_aidd_gate["checks"][0]["paths"], ["src/**/*.py", "lib/**/*.py", "tests/**/*.py"])
        imports = from_aidd_gate["checks"][1]
        self.assertEqual(imports["options"], {"roots": ["src", "lib"], "allow_modules": ["numpy"]})
        commands = [check for check in from_aidd_gate["checks"] if check["kind"] == "command"]
    ...
    def test_python_profile_without_source_root_uses_repository_scope(self):
        ...
        self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
        policy = (self.root / "aidd-gate.toml").read_text(encoding="utf-8")
    ...
    def test_invalid_command_fails_before_filesystem_mutation(self):
        ...
        self.assertFalse((self.root / "aidd-gate.toml").exists())
    ...
    def test_quoted_punctuation_is_literal_but_unquoted_operator_is_rejected(self):
        ...
        self.assertIn('"a|b"', (self.root / "aidd-gate.toml").read_text(encoding="utf-8"))
        ...
        self.assertFalse((other_root / "aidd-gate.toml").exists())
    ...
    def test_empty_command_and_sync_generation_options_fail_without_mutation(self):
        ...
        policy_before = (self.root / "aidd-gate.toml").read_bytes()
        ...
        self.assertEqual((self.root / "aidd-gate.toml").read_bytes(), policy_before)
